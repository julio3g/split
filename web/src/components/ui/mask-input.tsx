...
const NUMERIC_MASK_PATTERNS =
  /^(phone|zipCode|zipCodeExtended|brZipCode|cpf|cnpj|brPhone|ssn|ein|time|date|creditCard|creditCardExpiry)$/
...
const REGEX_CACHE = {
  ...
  // telefone BR: 10 (fixo) ou 11 (celular)
  brPhone: /^\d{10,11}$/,
...
// }
interface MaskPattern {
  // pattern: string | ((unmasked: string) => string)
  pattern: string
  transform?: (value: string, opts?: TransformOptions) => string
  validate?: (value: string, opts?: ValidateOptions) => boolean
}
...
const MASK_PATTERNS: Record<MaskPatternKey, MaskPattern> = {
  ...
  brZipCode: {
    pattern: '#####-###',
    transform: value => value.replace(REGEX_CACHE.nonDigits, ''), // mantém só dígitos
    validate: value =>
      REGEX_CACHE.brZipCode.test(value.replace(REGEX_CACHE.nonDigits, '')),
  },
  cpf: {
    pattern: '###.###.###-##',
    transform: value => value.replace(REGEX_CACHE.nonDigits, ''),
    validate: value =>
      REGEX_CACHE.cpf.test(value.replace(REGEX_CACHE.nonDigits, '')),
  },
  cnpj: {
    pattern: '##.###.###/####-##',
    transform: value => value.replace(REGEX_CACHE.nonDigits, ''),
    validate: value =>
      REGEX_CACHE.cnpj.test(value.replace(REGEX_CACHE.nonDigits, '')),
  },
  brPhone: {
    pattern: '(##) #####-####', // default (fixo)
    transform: value => value.replace(REGEX_CACHE.nonDigits, ''),
    validate: value =>
      REGEX_CACHE.brPhone.test(value.replace(REGEX_CACHE.nonDigits, '')),
  },
  currency: {
    pattern: '$###,###.##',
    transform: (
...
function getEffectivePattern(opts: {
  mask: MaskPatternKey | MaskPattern | undefined
  unmasked: string
  fallback: string
}) {
  const { mask, unmasked, fallback } = opts

  // só pro nosso telefone BR
  if (mask === 'brPhone') {
...
function MaskInput(props: MaskInputProps) {
  ...
  //   : maxLength
  const effectivePatternForLimits = React.useMemo(() => {
    ...
    return getEffectivePattern({
      mask: typeof mask === 'string' ? mask : undefined,
      unmasked: currentUnmasked,
      fallback: maskPattern.pattern,
    })
  }, [maskPattern, mask, value, transformOpts])

  const tokenCount = React.useMemo(() => {
    if (
      !effectivePatternForLimits ||
      CURRENCY_PERCENTAGE_SYMBOLS.test(effectivePatternForLimits)
    )
      return undefined
    return effectivePatternForLimits.match(REGEX_CACHE.hashPattern)?.length ?? 0
  }, [effectivePatternForLimits])

  const calculatedMaxLength = tokenCount
    ? effectivePatternForLimits?.length
    : maxLength
